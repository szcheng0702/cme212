...
class SpaceSearcher
{
  ...
  *  so that we can sort the morton_pair based on the magnitude of morton code*/
  struct mccomp {
	  bool operator()(morton_pair p1, morton_pair p2) {
		  return p1.code_ < p2.code_;
	  }
  };

  /* Functor to map point p to its MortonCode*/
  struct get_mc {
	  get_mc(MortonCoderType mc) :mc_(mc) {}
  ...
  SpaceSearcher(const Box3D& bb,
    ...
    // HW4: YOUR CODE HERE
	  ...
	  /* Build MortonCode Iterator from the Point Iterator*/
	  typedef thrust::transform_iterator<get_mc, PointIter, code_type> CodeIter;
      CodeIter cfirst(pfirst, get_mc(mc_));
	  CodeIter clast (plast, get_mc(mc_));

	  /* Build the zip_iterator for the morton_pair from the CodeIter and TIter*/
	  typedef thrust::zip_iterator<thrust::tuple<CodeIter, TIter>> ZipIter;
	  ZipIter zfirst (thrust::make_tuple(cfirst, tfirst));
	  ZipIter zlast (thrust::make_tuple(clast, tlast));

	  //Construct a standard vector storing the morton_pair
	  std::vector<morton_pair> z_data_(zfirst, zlast);

	  //sort the vector based on mccomp()
	  thrust::sort(thrust::omp::par, z_data_.begin(), z_data_.end(),mccomp());
  }
 ...
 private:
  ...
  struct morton_pair {
	...
	//Constructor
	morton_pair(thrust::tuple<code_type, value_type> pair)
